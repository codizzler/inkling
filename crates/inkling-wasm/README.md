...
`reveal.widths()` gives each cell's display width, so a canvas or a monospace grid can
reserve two columns for a wide glyph before it appears. `reveal.frame(progress)` renders
one frame to a string with the same engine the terminal renderers use, if you would rather
not walk the ranks yourself. `ease(curve, t)` exposes the timing curves.
